...
REQUIRED_FILES = (
    ...
    "tests/README.md", "docs/compatibility.md",
...
class RepositoryChecks(unittest.TestCase):
    def test_security_contact_and_maintainer_routing(self):
        for suffix in (".md", ".zh-CN.md"):
            ...
            community = (REPO_ROOT / f"docs/community/README{suffix}").read_text()
            contributing = (REPO_ROOT / f"CONTRIBUTING{suffix}").read_text()
            for content in (security, community):
                self.assertIn("mailto:vbot-edu@vbot.cn", document_links(content))
            self.assertIn(".github/CODEOWNERS", document_links(security))
            self.assertIn(".github/CODEOWNERS", document_links(contributing))
            self.assertIn(f"SECURITY{suffix}", document_links(contributing))
            self.assertNotIn("contact has not yet been published", security)
            self.assertNotIn("尚未公布专门的安全报告联系方式", security)
        codeowners = (REPO_ROOT / ".github/CODEOWNERS").read_text()
        rules = [line.split() for line in codeowners.splitlines()
        ...
        self.assertTrue(all(re.fullmatch(r"@[A-Za-z0-9][A-Za-z0-9-]*", owner)
                            for owner in owners))

    def test_compatibility_docs_live_in_developer_guide(self):
        self.assertFalse((REPO_ROOT / "release").exists())
        for suffix in (".md", ".zh-CN.md"):
            guide = REPO_ROOT / f"docs/compatibility{suffix}"
            content = guide.read_text()
            links = document_links(content)
            self.assertIn("https://github.com/VitaDynamics/vbot-lab/releases", links)
            for artifact in ("EDU_SDK_MANIFEST.json", "SHA256SUMS"):
                self.assertIn(artifact, content)
            for language in ("python", "cpp"):
                self.assertIn(f"../packages/aorta/{language}/README{suffix}", links)
            for origin in ("README", "AGENTS", "docs/README", "catalog/README",
                           "packages/README", "schemas/README", "docs/getting-started/README"):
                source = REPO_ROOT / f"{origin}{suffix}"
    ...
    def test_documented_application_target_matches_catalog(self):
        profile = json.loads((REPO_ROOT / "catalog/robots/foot_quadruped.json").read_text())
        self.assertEqual(profile["application_version"], "V1.6.0")
        pages = (
            "README", "AGENTS", "catalog/README", "docs/compatibility", "recipes/README",
            "docs/robots/foot_quadruped/README", "docs/interfaces/README",
            "docs/interfaces/aorta-ros2", "docs/interfaces/audio", "docs/interfaces/perception",
            "docs/interfaces/agent/README", "docs/interfaces/agent/customization",
        )
        for suffix in (".md", ".zh-CN.md"):
            for name in pages:
                with self.subTest(page=name, language=suffix):
                    content = (REPO_ROOT / f"{name}{suffix}").read_text()
                    self.assertIn(profile["application_version"], content)
                    self.assertNotRegex(content, r"(?:minimum[^\n]*version[^\n]*not yet published|最低[^\n]*版本尚未公布|尚未公布最低)")
            matrix = (REPO_ROOT / f"docs/compatibility{suffix}").read_text()
            self.assertNotIn("v5.0.21-", matrix)
            self.assertIn("edu-sdk-2026.9.24", matrix)
            self.assertIn("2026.9.24-v4", matrix)

    def test_back_mounting_reference_and_drawing_integrity(self):
        directory = REPO_ROOT / "docs/hardware/quadruped-common"
        assets = directory / "assets/back-mounting"
        # Preserve the reviewed engineering drawings and CAD geometry. PDFs have
        # export-workstation metadata removed; no PDF/CAD dependency is needed in CI.
        digests = {
            "back-mounting-holes.pdf": "d539f40de55118cecf5f91bedd78257d0be54f3fcf8d559b02eb19b14e6fc0ba",
            "arm-adapter-plate.pdf": "b4bd4e96aed0406d1582bded182eddde0cf9be54c0c78712e249ccdbd5713018",
            "arm-adapter-plate.step": "3f938b77ecb12e22f6b328a67bc5d5af603f591f1fe06600887e89815b6cb43e",
            "arm-installation.jpeg": "9a205a35bb39b070d9390a961aa303f08c2984851d526c6d5f92b8d878b3dc85",
        }
        self.assertEqual(sorted(p.name for p in assets.iterdir()), sorted(digests))
        ...
        dimensions = []
        for suffix in (".md", ".zh-CN.md"):
            guide = directory / f"back-mounting{suffix}"
            content = guide.read_text()
            links = document_links(content)
            for name in digests:
                self.assertIn(f"assets/back-mounting/{name}", links)
            self.assertIn(f"../../../assets/robots/README{suffix}", links)
            self.assertIn(f"../../community/README{suffix}", links)
            for identifier in ("foot_quadruped", "wheel_quadruped", "foot_humanoid",
                               "M4×12", "M5×10", "M5-6H", "Ø4.50", "Ø4.20"):
                self.assertIn(identifier, content)
            dimensions.append(re.findall(r"(?<![A-Za-z0-9])[-+]?\d+(?:\.\d+)?", content))
            for origin in (f"AGENTS{suffix}", f"docs/hardware/README{suffix}",
                           f"docs/robots/foot_quadruped/README{suffix}",
                           f"docs/robots/wheel_quadruped/README{suffix}"):
                source = REPO_ROOT / origin
                targets = [(source.parent / link).resolve()
                           for link in document_links(source.read_text())
                           if not urlsplit(link).scheme]
                self.assertIn(guide.resolve(), targets, origin)
            self.assertFalse((REPO_ROOT / f"docs/hardware/foot_quadruped/back-mounting{suffix}").exists())
            bipedal = (REPO_ROOT / f"docs/robots/foot_humanoid/README{suffix}").read_text()
            self.assertFalse(any("back-mounting" in link for link in document_links(bipedal)))
        self.assertEqual(dimensions[0], dimensions[1], "Keep dimensions and fastener counts aligned")
    ...
    def test_community_entry_points_and_issue_chooser(self):
        ...
        self.assertEqual(re.findall(r"^\s+url:\s+(\S+)$", chooser, re.M),
                         urls + ["https://github.com/VitaDynamics/vbot-lab/security/policy"])
        for suffix in (".md", ".zh-CN.md"):
