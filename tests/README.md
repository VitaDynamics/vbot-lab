...
Viewer/community navigation and issue-chooser links are checked locally. Model metadata
must keep the reserved `foot_quadruped` resources explicitly unavailable while no URDF
or meshes are supplied; the checks do not contact the Viewer or post to the forum.
