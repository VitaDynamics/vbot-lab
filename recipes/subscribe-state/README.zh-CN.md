...
接口: `/locomotion/status` — pub/sub.

生成类型: `locomotion.LocomotionStatus`.

输出心跳序号、姿态、运动状态、当前动作及时间戳。姿态：0 未知、1 站立、2 趴下、3 过渡中；运动状态：0 空闲、1 恢复中、2 紧急状态。该示例只读；收到样本或心跳变化不代表运动完成。超时未收到足够样本会报错。
...
按[二进制部署流程](../../packages/aorta/cpp/README.zh-CN.md#在机器人上运行)设置 RECIPE=subscribe-state 并完成部署。以 vbot 身份 SSH 登录后，进入部署所得应用目录，按流程配置 shell 和动态库路径，满足上面的任务前提后直接运行可执行文件：
