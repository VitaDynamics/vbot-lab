...
仓库管理员共同组成维护团队，负责问题分流、贡献评审和兼容性变更协调。
[CODEOWNERS](.github/CODEOWNERS) 列出所有仓库路径的评审负责人；管理员成员变化时同步更新。
评审分配不会授予仓库权限，也不替代已配置的合并要求。

安全问题请按[安全说明](SECURITY.zh-CN.md)私下反馈，不要使用公开 Issue 或 PR。
