...
<p align="center"><a href="expressions.md">English</a> | 中文</p>

调用方式选择与字段对应见[直接服务与 RCP](../guides/control-paths.zh-CN.md)。

下列为显示表情，不是身体动作。前三位数字即 `emotion_id`，如 `048_say_hi` 对应 48。
显式非零 ID 优先于 method 映射。ID 0 使用 `BLINK_ONCE`／`emotion.BLINK_ONCE`，
不要用其他 method 搭配 0 来强制选择眨眼。
`EmotionMethod` 是另一套枚举，数值不是资源 ID；例如 `HAPPY` 对应 19，`SAY_HI` 对应 48。
资源名不一定等于 DSL 别名，`emotion.HAPPY` 和 `emotion.happy` 的含义也不同。
未覆盖在命名枚举中的表情可通过公开的 `emotion_id` 字段选择。
持有时长与显示层按编写指南设置。保留资源名的原有拼写；不要用模拟充电、安全或故障提示误导他人。
