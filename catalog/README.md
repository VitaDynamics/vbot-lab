...
See [offline tests](../tests/README.md) for validation and [compatibility](../release/compatibility.md)
...
sources when integrated. Firmware/runtime version requirements still need device verification.
...
`implemented` means an executable entry exists, not that a device is connected. Unknown minimum
firmware versions still require route checks on the selected device.
